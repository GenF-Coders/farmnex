...
class NotificationService:
    ...
    async def get(self, public_id: UUID) -> Notification:
        entity = await self.repository.get_by_public_id(public_id)
        if entity is None:
            raise NotFoundError("Notification not found.")
        return entity

    async def list(self, offset: int = 0, limit: int = 100) -> tuple[list[Notification], int]:
        if offset < 0:
    ...
    async def update(self, public_id: UUID, data: dict[str, Any]) -> Notification:
