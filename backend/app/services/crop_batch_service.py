...
class CropBatchService:
    ...
    async def create(self, data: dict[str, Any]) -> CropBatch:
        return await self.repository.create(**data)

    async def get(self, public_id: UUID) -> CropBatch:
        entity = await self.repository.get_by_public_id(public_id)
        if entity is None:
            raise NotFoundError("CropBatch not found.")
        return entity

    async def list(self, offset: int = 0, limit: int = 100) -> tuple[list[CropBatch], int]:
        if offset < 0:
    ...
    async def update(self, public_id: UUID, data: dict[str, Any]) -> CropBatch:
